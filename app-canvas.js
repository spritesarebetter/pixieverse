...
function artColor(x,y){let color=null;for(let s of fr().sprites){if(!s.visible)continue;let lx=x-s.ox,ly=y-s.oy;if(lx<0||ly<0||lx>=sz()||ly>=sz()||!s.mask[ly][lx])continue;let a=s.lines[ly];if(s.transparent&&a.color===0)continue;if(a.or){if(color!==null)color=(color|a.color)&15}else if(color===null)color=a.color}return color}
...
function put(a,b,v){let la=layerPoint(a),lb=layerPoint(b);if(!inLayer(la)&&!inLayer(lb)){setStatus('Outside selected sprite');return}let x0=la.x,y0=la.y,x1=lb.x,y1=lb.y,dx=Math.abs(x1-x0),sx=x0<x1?1:-1,dy=-Math.abs(y1-y0),sy=y0<y1?1:-1,er=dx+dy;for(;;){if(x0>=0&&y0>=0&&x0<sz()&&y0<sz())layer().mask[y0][x0]=v;if(x0===x1&&y0===y1)break;let e2=2*er;if(e2>=dy){er+=dy;x0+=sx}if(e2<=dx){er+=dx;y0+=sy}}dirty()}
