...
function artColor(x,y){let color=null;for(let s of fr().sprites){if(!s.visible)continue;let lx=x-s.ox,ly=y-s.oy;if(lx<0||ly<0||lx>=sz()||ly>=sz()||!s.mask[ly][lx])continue;let a=s.lines[ly];if(!a.color)continue;if(a.or){if(color!==null)color=(color|a.color)&15}else if(color===null)color=a.color}return color}
