...
function mkLayer(i){const maxPattern=(P?.size||16)===16?63:255;return{name:'Sprite '+i,ox:0,oy:0,pattern:C(Math.round(Number(i)||0),0,maxPattern),priority:Math.max(0,Math.round(Number(i)||0)),visible:true,transparent:true,mask:mask(),lines:attrs()}}
function mkFrame(i){return{name:'Frame '+i,wait:6,sprites:[mkLayer(0)]}}
function spritePriority(s,index=0){const p=Number(s?.priority);return Number.isFinite(p)?Math.max(0,Math.round(p)):Math.max(0,Math.round(index)||0)}
function normalizeFramePriorities(f){
  if(!f?.sprites?.length)return;
  const ranked=f.sprites.map((s,index)=>({s,index,priority:spritePriority(s,index)})).sort((a,b)=>a.priority-b.priority||a.index-b.index);
  ranked.forEach((entry,priority)=>entry.s.priority=priority);
  const bottom=ranked[0]?.s;if(bottom?.lines)bottom.lines.forEach(a=>a.or=false);
}
function spritesByPriority(frame,topFirst=false){
  const ranked=(frame?.sprites||[]).map((s,index)=>({s,index,priority:spritePriority(s,index)}));
  ranked.sort((a,b)=>topFirst?(b.priority-a.priority||a.index-b.index):(a.priority-b.priority||a.index-b.index));
  return ranked;
}
function setSpritePriority(frame,index,target){
  if(!frame?.sprites?.[index])return false;
  normalizeFramePriorities(frame);
  const s=frame.sprites[index],old=s.priority,next=C(Math.round(Number(target)||0),0,frame.sprites.length-1);
  if(next===old)return false;
  const other=frame.sprites.find((peer,i)=>i!==index&&peer.priority===next);
  if(other)other.priority=old;
  s.priority=next;
  normalizeFramePriorities(frame);
  return true;
}
...
function parseProject(raw){
  ...
  const size=+raw.size,palette=normalizePalette(raw.palette),frames=raw.frames.map((f,fi)=>{
    ...
    const out={name:String(f.name||('Frame '+fi)),wait:C(Math.round(Number(f.wait)||6),1,9999),sprites:f.sprites.map((s,i)=>{
      ...
      const rawPriority=Number(s.priority),priority=Number.isFinite(rawPriority)?Math.max(0,Math.round(rawPriority)):i;
      return{name:String(s.name||('Sprite '+i)),ox:Math.round(Number(s.ox)||0),oy:Math.round(Number(s.oy)||0),pattern:C(Math.round(Number(s.pattern)||0),0,size===16?63:255),priority,visible:s.visible!==false,transparent:s.transparent!==false,mask:m,lines};
    })};
