...
(() => {
  ...
  const animationCanvas=$('animationPreviewCanvas'),animationWrap=$('animationPreviewWrap');
  ...
  function drawAnimationPreview(frame){drawFrameInto(animationCanvas,frame,256,144);if(animationCanvas)animationCanvas.setAttribute('aria-label','Animation preview · '+String(frame?.name||''))}
  ...
  renderFrames=function(){
    ...
    P.frames.forEach((f,i)=>{
      ...
      d.append(head,thumb,w);d.onclick=()=>{F=i;S=0;L=0;render();if(!playing)drawAnimationPreview(fr())};
      d.ondragstart=e=>{dragIndex=i;d.classList.add('dragging');e.dataTransfer.effectAllowed='move';e.dataTransfer.setData('text/plain',String(i));stop(false)};d.ondragover=e=>{e.preventDefault();e.dataTransfer.dropEffect='move';d.classList.add('dragover')};d.ondragleave=()=>d.classList.remove('dragover');
  ...
  $('addFrame').onclick=()=>{stop(false);baseAdd();drawAnimationPreview(fr())};$('dupFrame').onclick=()=>{stop(false);baseDup();drawAnimationPreview(fr())};$('delFrame').onclick=()=>{stop(false);baseDel();drawAnimationPreview(fr())};
  $('playFrames').onclick=play;$('stopFrames').onclick=()=>stop();$('slowerFrames').onclick=()=>changeSpeed(-10);$('fasterFrames').onclick=()=>changeSpeed(10);
