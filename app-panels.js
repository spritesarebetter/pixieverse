...
(() => {
  ...
  function stableFitNow(force=false){
    if(!editorWrap||!stage||fitting||(userManualZoom&&!force))return;
    const unit=stage.querySelector('.spriteunit');if(!unit)return;fitting=true;
    try{
      const available=Math.max(70,editorWrap.clientHeight-14);let lo=.1,hi=8,best=.1;
      for(let i=0;i<11;i++){
        const midZoom=(lo+hi)/2;editorZoom=midZoom;applyEditorScale();const h=unit.getBoundingClientRect().height;
        if(h<=available){best=midZoom;lo=midZoom}else hi=midZoom;
      }
  ...
  window.fitObjectEditorSprites=(iterations,force=false)=>queueStableFit(force);
  window.resetObjectEditorFit=()=>{userManualZoom=false;queueStableFit(true)};
  ...
  function setTimelineHeight(value,save=true){
    if(!mid)return;const lim=timelineLimits(),h=C(Math.round(Number(value)||145),lim.min,lim.max);mid.style.setProperty('--timeline-height',h+'px');if(save)try{localStorage.setItem(TIMELINE_HEIGHT_KEY,String(h))}catch(_){}queueStableFit();window.redrawAnimationPreview?.();
  }

  function setCollapsed(section,collapsed){
    section.classList.toggle('collapsed',collapsed);const button=section.querySelector(':scope > .sectionhead .collapseToggle, :scope > .timelinehead .collapseToggle');if(!button)return;
    button.setAttribute('aria-expanded',String(!collapsed));button.textContent=collapsed?'▸':'▾';const title=section.querySelector('h2')?.textContent?.trim()||'menu';button.title=(collapsed?'Show ':'Hide ')+title;
    if(!collapsed)requestAnimationFrame(()=>{window.redrawPreviewNow?.();window.syncAllSpriteScales?.();queueStableFit();window.redrawAnimationPreview?.()});
  }
  ...
  function setPreviewWidth(value,save=true){
    if(!main||window.matchMedia('(max-width:800px)').matches)return;const lim=limits(),w=C(Math.round(Number(value)||360),lim.min,lim.max);main.style.setProperty('--preview-width',w+'px');if(save)try{localStorage.setItem(WIDTH_KEY,String(w))}catch(_){}requestAnimationFrame(()=>{window.redrawPreviewNow?.();queueStableFit();window.redrawAnimationPreview?.()});
  }
  if(main&&splitter){
    let saved=360;try{saved=Number(localStorage.getItem(WIDTH_KEY))||360}catch(_){}setPreviewWidth(saved,false);
    ...
    const stop=e=>{if(!splitDrag)return;splitDrag=false;splitter.classList.remove('dragging');setPreviewWidth(currentWidth(),true);try{splitter.releasePointerCapture?.(e.pointerId)}catch(_){}};['pointerup','pointercancel','lostpointercapture'].forEach(ev=>splitter.addEventListener(ev,stop));
  ...
  if(mid&&timelineSplitter){
    let saved=145;try{saved=Number(localStorage.getItem(TIMELINE_HEIGHT_KEY))||145}catch(_){}setTimelineHeight(saved,false);let dragging=false,startY=0,startHeight=145;
    timelineSplitter.addEventListener('pointerdown',e=>{if(e.button!==0)return;e.preventDefault();dragging=true;startY=e.clientY;startHeight=currentTimelineHeight();timelineSplitter.classList.add('dragging');timelineSplitter.setPointerCapture?.(e.pointerId)});
    ...
    const stopTimeline=e=>{if(!dragging)return;dragging=false;timelineSplitter.classList.remove('dragging');setTimelineHeight(currentTimelineHeight(),true);try{timelineSplitter.releasePointerCapture?.(e.pointerId)}catch(_){}};['pointerup','pointercancel','lostpointercapture'].forEach(ev=>timelineSplitter.addEventListener(ev,stopTimeline));
  ...
  window.addEventListener('resize',()=>{setPreviewWidth(currentWidth(),false);setTimelineHeight(currentTimelineHeight(),false);window.redrawPreviewNow?.();queueStableFit();window.redrawAnimationPreview?.()});
  requestAnimationFrame(()=>queueStableFit(true));
})();
